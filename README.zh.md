...
然后在任意已有（或将要有）`flightdeck/` 目录的项目中启动 Claude Code 会话：协议自动加载，读取 `flightdeck/cockpit.md`，与 `git status` 对账，无缝衔接之前的工作。

全新项目可用 `/flightdeck:workflow` 通过两个问题完成初始化，自动生成 `flightdeck/cockpit.md`。
...
安装后，只要项目中存在 `flightdeck/` 目录，skill 即自动加载；也可以手动调用。
...
flightdeck 检测到目录缺失，征询确认后，通过两个简短问题（Active focus、Next session 第一条）生成 `flightdeck/cockpit.md`。完成。自下一次会话起，SessionStart hook 会自动加载 skill。
...
| 命令 | 自动加载？ | 用途 |
| --- | --- | --- |
| `/flightdeck:workflow` | ✅ 项目有 `flightdeck/` 时 SessionStart hook 自动注入 | 主协议。也负责 bootstrap：没目录时引导填 cockpit.md。 |
| `/flightdeck:preflight` | — 仅显式 | 长 session 偏题后重新锚回 `cockpit.md` + git 状态。 |
| `/flightdeck:landing` | — 仅显式 | Session 收尾 —— 分类新知识、更新 cockpit、可选 commit。 |
| `/flightdeck:walkaround` | — 仅显式 | 10 类完整性审计 —— 协议漂移检测。 |
| `/flightdeck:emit-agents-md` | — 仅显式 | 从 `cockpit.md` 在 fenced markers 之间重生 `AGENTS.md`。 |

除 `workflow` 外，所有命令都带 `disable-model-invocation: true` —— 只在用户显式打 slash 时触发。
...
说"收尾"之类的，AI 跑 [landing ritual](skills/workflow/exit-ritual.md)：
...
对 —— 就是这个意思。协议是 `skills/workflow/SKILL.md`，AI 加载它。状态是 `flightdeck/` 下面的纯 markdown。没数据库、没服务器、不用部署任何东西。可以在代码评审里 diff 它、在终端里 grep 它、在编辑器里改它。AI 工具是协议的**参与者**，而非它的保管者。
...
| 文件 | 覆盖内容 |
| --- | --- |
| [skills/workflow/SKILL.md](skills/workflow/SKILL.md) | AI 加载的入口 —— 协议、权威序、生命周期、设计哲学 |
| [skills/workflow/folder-semantics.md](skills/workflow/folder-semantics.md) | 每个文件夹的语义和职责；minimal vs full；future expansion slots |
| [skills/workflow/templates.md](skills/workflow/templates.md) | incident / checklist / sketch / debrief / cockpit 模板含 frontmatter 规则 |
| [skills/workflow/exit-ritual.md](skills/workflow/exit-ritual.md) | Landing ritual —— 分类启发式、red flags、晋升门 |
