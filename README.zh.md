...
新项目用 `/flightdeck:workflow` 走两问 bootstrap，自动生成 `flightdeck/cockpit.md`。
...
整个入场体验就这些。没有 500 行的上下文倾倒。没有 AI 每次都要扫一遍的项目背景介绍。**80 行，硬规则。** 历史性 / 情境性的内容都在更深一层 —— 先读 `INDEX.md` 定位，再按需打开具体文件。
...
对 —— 就是这个意思。协议是 `skills/workflow/SKILL.md`，AI 加载它。状态是 `flightdeck/` 下面的纯 markdown。没数据库、没服务器、不用部署任何东西。代码评审里 diff 它、终端里 grep 它、编辑器里改它。AI 工具是协议的**参与者**，不是它的看门人。
...
撞到天花板就强制做真实决策：这条内容配得上待在入口文件吗？还是该挪到更深的文件夹（`specs/`、`incidents/`）或 `## Hanging tasks`，或者干脆移出？
